...
namespace NodeCanvas.Tasks.Actions 
{
	...
	public class GetCardsDataFromContainer : ActionTask<CardContainerBase>
	{
		...
		[BlackboardOnly] public BBParameter<List<float>> selfAttackTableList =
			new BBParameter<List<float>>() { name = "Self Attack Table List" };
		[BlackboardOnly] public BBParameter<List<float>> selfDefenceTableList =
			new BBParameter<List<float>>() { name = "Self Defence Table List" };
		...
		private void HasTablesChanged(List<float> oldAttackTable, List<float> oldDefenceTable)
		{
